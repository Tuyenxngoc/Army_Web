...
try {
    ...
    if ($xuBefore > 2000000000) {
        echo '{"code": "01", "text": "Số xu hiện tại đã vượt quá 2 tỷ."}';
        return;
    }
    ...
    foreach ($configDoiXu as $item) {
        if ($item['pCoin'] == $pCoin) {
            $xuChange = $item['xu'] + ($item['xu'] * $bonusDoiXu['bonus'] / 100);
            $xuAfter += $xuChange;
            break;
        }
    }
    if ($xuAfter > 2000000000) {
        echo '{"code": "01", "text": "Số xu không được vượt quá 2 tỷ."}';
        return;
    }
    ...
    if ($balanceBefore >= $pCoin || $pCoin == 0) {
        ...
        $sqlUpdateArmymem = 'UPDATE armymem SET xu = ' . $xuAfter . ' WHERE id = ' . $userId;
