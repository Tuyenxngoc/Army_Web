...
$exchangesXu = [];
for ($i = 0; $i < count($configDoiXu); $i++) {
    array_push($exchangesXu, new ExchangeXu($configDoiXu[$i]['pCoin'], $configDoiXu[$i]['xu']));
}
?>
<div class="overlay"></div>
<div class="text-center fw-semibold fs-5" style="color:#333 !important;">Đổi Coin ra Xu
    <span class="text-danger"><?php echo $bonusDoiXu['bonus'] > 0 ? "(KM " . $bonusDoiXu['bonus'] . "%)" : "" ?></span>
</div>
              ...
              <div class="col">
                  <div class="w-100 fw-semibold cursor-pointer" onclick="handleClick(' . $exc->pcoin . ')">
                      <div id="button-' . $exc->pcoin . '" class="recharge-method-item false" style="height: 90px;">
                          ...
                          <div class="text-danger">' . number_format($exc->xu + ($exc->xu) * ($bonusDoiXu['bonus'] / 100)) . ' Xu</div>
                      </div>
                  </div>
              </div>
          </div>';
...
<script>
    ...
    let configDoiXu = <?php echo json_encode($configDoiXu); ?>;
    let bonus = <?php echo $bonusDoiXu['bonus']; ?>;
    ...
    function handleClick(pcoin) {
        ...
        let selectedConfig = configDoiXu.find(config => config.pCoin === selected);
        let xu = selectedConfig ? selectedConfig.xu : 0;

        let requiredAmount = xu + (xu * (bonus / 100));
